...
const EVENT_STYLE: Record<EventType, { color: string; bg: string; icon: string }> = {
  PLAN_CREATED:       { color: "text-amber-300",   bg: "bg-amber-500/10",   icon: "◈" },
  SEARCH_DONE:        { color: "text-sky-400",      bg: "bg-sky-500/10",     icon: "⌕" },
  SOURCES_COLLECTED:  { color: "text-emerald-400",  bg: "bg-emerald-500/10", icon: "◎" },
  SUMMARY_CHUNK:      { color: "text-orange-400",   bg: "bg-orange-500/10",  icon: "◑" },
  SUMMARY_DONE:       { color: "text-orange-300",   bg: "bg-orange-500/8",   icon: "◉" },
  REPORT_CHUNK:       { color: "text-amber-400",    bg: "bg-amber-500/10",   icon: "◆" },
  REPORT_DONE:        { color: "text-amber-300",    bg: "bg-amber-500/12",   icon: "◆" },
...
function EventDetail({ e }: { e: WSEvent }) {
  switch (e.event) {
        ...
        <ul className="mt-1 space-y-0.5">
          {sources.slice(0, 3).map((s, i) => (
            <li key={i} className="flex min-w-0 items-start gap-1.5">
              <span className="mt-px shrink-0 text-emerald-700">·</span>
          ...
          {sources.length > 3 && (
            <li className="text-stone-700">+{sources.length - 3} more</li>
          )}
        </ul>
      )
    }
    case "SUMMARY_CHUNK": {
      ...
      return chunk ? (
        <p className="line-clamp-2 text-stone-500">{chunk}</p>
      ) : null
    }
    case "REPORT_DONE": {
      ...
      return <p className="text-stone-500">{words.toLocaleString()} words synthesized</p>
    }
    case "ERROR": {
  ...
  if (filtered.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3">
        ...
        <p className="font-mono text-xs text-stone-700">Awaiting your query...</p>
      </div>
    )
  }
          ...
          {filtered.map((e, i) => {
            ...
            return (
              ...
              >
                {/* Timeline dot */}
                <div
                  className={`relative z-10 mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-stone-800 bg-stone-950 font-mono text-[10px] ${style.color}`}
                >
                  {style.icon}
                </div>
                ...
                <div className={`min-w-0 flex-1 rounded-lg p-2.5 font-mono text-[11px] ${style.bg}`}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-auto shrink-0 text-stone-700">
                  ...
                  <div className="mt-1.5 flex items-center gap-1">
                    ...
                    {e.subtopic && (
                      <span className="rounded bg-amber-950/40 px-1.5 py-0.5 text-[9px] text-amber-700">
